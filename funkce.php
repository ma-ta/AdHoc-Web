...
function posli_potvrzeni($jmeno, $email, $telefon, $profil, $heslo, $castka, $zprava) {
    ...
    $predmet = "Potvrzovací e-mail (" . DOMENA . ")";
    ...
    try {
        ...
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        $headers .= "From: <" . $email . ">" . "\r\n";
        mail(KOPIE_1, "=?UTF-8?B?" . base64_encode("Fwd: " . $predmet) . "?=", $text, $headers);
        */
    } catch (Exception $ex) {
...
// není asynchronní – problém se zamrzáním stránky (nepoužívat či upravit)
function kurzy($mena) {
    if ($mena == "btc") {
        $adresa = "http://www.bitstamp.net/api/ticker/";
        try {
            $file = fopen($adresa, "r");
            while (!feof($file)) {
